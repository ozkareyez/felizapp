...
function drawGradientBar(doc, x, y, w, h) {
  ...
  for (let i = 0; i < strips; i++) {
    const t = i / (strips - 1)
    const colIdx = t * (GRADIENT.length - 1)
    const idxA = Math.floor(colIdx)
    const idxB = Math.min(idxA + 1, GRADIENT.length - 1)
    const frac = colIdx - idxA
    const ca = GRADIENT[idxA]
    const cb = GRADIENT[idxB]
...
function drawGradientRounded(doc, x, y, w, h, r) {
  const strips = 60
  const sw = w / strips
  ...
  for (let i = 0; i < strips; i++) {
    ...
    let bottomClip = 0
    if (dl < r) {
      const c = r - Math.sqrt(r * r - dl * dl)
      topClip = c
      bottomClip = c
    }
    if (dr < r) {
      const c = r - Math.sqrt(r * r - dr * dr)
      topClip = Math.max(topClip, c)
      bottomClip = Math.max(bottomClip, c)
    }
    if (topClip + bottomClip >= h) continue
    const t = i / (strips - 1)
    const colIdx = t * (GRADIENT.length - 1)
    const idxA = Math.floor(colIdx)
    const idxB = Math.min(idxA + 1, GRADIENT.length - 1)
    ...
    doc.setFillColor(cr, cg, cb2)
    doc.rect(sx, y + topClip, sw + 0.5, h - topClip - bottomClip, "F")
  }
}
...
function drawBlueTexture(doc) {
  ...
  for (let x = 10; x < 210; x += 16) {
    for (let y = 10; y < 297; y += 16) {
      const ci = ((x * 7 + y * 13) % GRADIENT.length)
      doc.setFillColor(...GRADIENT[ci])
  ...
  for (let x = 10; x < 210; x += 64) {
    for (let y = 10; y < 297; y += 64) {
      const ci = ((x * 3 + y * 11) % GRADIENT.length)
      doc.setFillColor(...GRADIENT[ci])
...
function drawTable(doc, startY, items, rentalDays, lbl) {
  ...
  // Header rounded box
  drawGradientRounded(doc, margin, startY, totalW, 9, r)
  doc.setDrawColor(...C.border)
  doc.setLineWidth(0.5)
...
function drawSummary(doc, x, y, rentalDays, subtotal, total, lbl) {
  ...
  drawRow("Subtotal", "AWG " + subtotal.toFixed(2))

  // Total gradient box
  yy += 2
  drawGradientRounded(doc, x, yy, w, 12, r)
  doc.setDrawColor(...C.border)
  doc.setLineWidth(0.5)
...
function drawThankYou(doc, y) {
  ...
  doc.setTextColor(...GRADIENT[1])
  doc.setFontSize(10)
  doc.setFont("helvetica", "italic")
  doc.text("— Gracias por su confianza —", 105, y + 7, { align: "center" })
}
