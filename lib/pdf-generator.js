...
export async function generateInvoicePDF(invoice, items, client, rentalDays = 1, locale = "es") {
  ...
  try {
    doc.addImage(logoUrl, "PNG", 12, 8, 25, 20)
  } catch (e) {
...
export async function generateQuotePDF(quote, items, client, rentalDays = 1, locale = "es") {
  ...
  doc.setFontSize(14)
  doc.setFont("helvetica", "bold")
  doc.text(companyInfo.name, 42, 18)
  
  doc.setFontSize(8)
  doc.setFont("helvetica", "normal")
  doc.setTextColor(...textGray)
  doc.text("Telf: " + companyInfo.phone, 42, 24)
  doc.text("Email: " + companyInfo.email, 42, 29)
  doc.text("RUC: " + companyInfo.ruc, 42, 34)
