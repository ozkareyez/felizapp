...
export async function getAniversaryBase64() {
  if (cachedAniversary) return cachedAniversary
  try {
    const res = await fetch('/api/aniversary')
    const data = await res.json()
    cachedAniversary = `data:image/png;base64,${data.img}`
    return cachedAniversary
  } catch {
    return null
  }
}
...
const footerText = {
  es: "Gracias por 10 grandes años.",
  en: "Thanks for 10 great years."
}
...
function drawTable(doc, startY, items, rentalDays, lbl) {
  ...
  // Header rounded box
  drawSolidRounded(doc, margin, startY, totalW, 9, r, C.primary)
  doc.setDrawColor(...C.border)
  doc.setLineWidth(0.5)
...
function drawPaymentInfo(doc, y, lbl, aniversaryBase64) {
  const r = 2
  const h = aniversaryBase64 ? 50 : 36
  doc.setFillColor(...C.card)
  roundedRect(doc, 20, y, 170, h, r, "F")
  doc.setDrawColor(...C.border)
  doc.setLineWidth(0.5)
  ...
  if (aniversaryBase64) {
    ...
    doc.setDrawColor(...C.divider)
    doc.setLineWidth(0.3)
    doc.line(midX, y + 4, midX, y + h - 4)

    doc.addImage(aniversaryBase64, "PNG", 108, y + 4, 80, 42)
  }

  doc.setTextColor(...C.text)
  doc.setFontSize(8)
  doc.setFont("helvetica", "bold")
  doc.text(lbl.bankData, aniversaryBase64 ? 32 : 28, y + 9)
  ...
  rows.forEach(([k, v]) => {
    doc.setTextColor(...C.textSecondary)
    doc.setFont("helvetica", "normal")
    doc.text(k + ":", aniversaryBase64 ? 32 : 28, py)
    doc.setTextColor(...C.text)
    doc.setFont("helvetica", "bold")
    doc.text(truncate(doc, v, aniversaryBase64 ? 68 : 95), aniversaryBase64 ? 68 : 66, py)
    py += 4
  })
}
...
function drawFooter(doc, ft) {
  ...
  for (let x = 0; x < 210; x += 6) {
    doc.setFillColor(...(x % 12 < 6 ? C.primary : C.primary))
...
export async function generateInvoicePDF(invoice, items, client, rentalDays = 1, locale = "es") {
  ...
  const aniversaryBase64 = await getAniversaryBase64()
  ...
  drawPaymentInfo(doc, y, lbl, aniversaryBase64)
  y += aniversaryBase64 ? 55 : 40
