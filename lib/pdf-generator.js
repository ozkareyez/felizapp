...
const footerText = {
  es: "Factura generada conforme a la regulacion fiscal de Aruba - DIMP | FELIZ ENTERPRISE © 2026",
...
const labels = {
  es: {
    ...
    invoiceFile: "Factura-", quoteFile: "Cotizacion-", date: "Fecha"
  },
  en: {
    ...
    invoiceFile: "Invoice-", quoteFile: "Quote-", date: "Date"
  }
}
...
export async function generateInvoicePDF(invoice, items, client, rentalDays = 1, locale = "es") {
  ...
  if (logoBase64) {
    ...
    doc.text("Telf: " + company.phone, 44, 24)
    doc.text("Email: " + company.email, 44, 29)
    doc.text("RUC: " + company.ruc, 44, 34)
  } else {
    doc.setTextColor(...textDark)
    doc.setFontSize(14)
    doc.setFont("helvetica", "bold")
    ...
    doc.setFont("helvetica", "normal")
    doc.setTextColor(...textGray)
    doc.text("Telf: " + company.phone, 15, 24)
    doc.text("Email: " + company.email, 15, 29)
    doc.text("RUC: " + company.ruc, 15, 34)
  }
  ...
  doc.text(lbl.date + ": " + new Date(invoice.created_at).toLocaleDateString(locale === "en" ? "en-US" : "es-ES"), 165, 31, { align: "center" })
...
export async function generateQuotePDF(quote, items, client, rentalDays = 1, locale = "es") {
  ...
  doc.text(lbl.date + ": " + new Date(quote.created_at).toLocaleDateString(locale === "en" ? "en-US" : "es-ES"), 162.5, 33, { align: "center" })
