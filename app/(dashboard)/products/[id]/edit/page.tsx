...
export default function EditProductPage() {
  ...
  const handleDelete = async () => {
    ...
    if (error) {
      alert(t("products.errorDelete") + ": " + error.message)
    } else {
      router.push("/products")
    }
  }
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-slate-500 font-medium">{t("products.loading")}</p>
        </div>
      </div>
    )
  }
      ...
      <div className="max-w-3xl mx-auto px-6">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-2 text-slate-500">
            ...
            <span className="text-sm">{t("products.editTitle")}</span>
          </div>
        </div>
        ...
        {saved && (
          <div className="mb-6 p-4 bg-emerald-50 border border-emerald-200 rounded-xl flex items-center gap-3 animate-fade-in">
            ...
            <span className="text-emerald-700 font-medium">{t("products.updatedSuccess")}</span>
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          ...
          <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50">
            <h1 className="text-xl font-bold text-slate-900">{name || t("products.editTitle")}</h1>
            <p className="text-sm text-slate-500 mt-1">{t("products.editSubtitle")}</p>
          </div>
          ...
          <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6 animate-scale-in">
              <div className="flex items-center gap-3 text-red-600 mb-4">
                ...
                <div>
                  <h3 className="text-lg font-bold">{t("products.deleteTitle")}</h3>
                  <p className="text-sm text-red-500">{t("products.sureDelete")}</p>
                </div>
              </div>
              
              <p className="text-slate-600 mb-6">
                {t("products.sureDelete")}
              </p>
